...
int start(){ 
   ...
   double value_sl = MarketInfo(Symbol(), MODE_ASK) - distance;
   double value_tp = MarketInfo(Symbol(), MODE_ASK) + distance;
   int value_cmd = 0;
   int temp_error;
   ...
   GetLastError();
   if(value_sl > 0){
      if(OrderSend(Symbol(), value_cmd, value_volume, MarketInfo(Symbol(), MODE_ASK), 20, value_sl, value_tp) == -1){
