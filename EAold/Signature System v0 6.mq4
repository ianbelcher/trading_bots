...
//#define PROGRAM_VERSION "v0 5" //Reveloped the lookback section. Accounting for more data.
#define PROGRAM_VERSION "v0 6" // Introduction of WinUser32.mqh. Major clean of code. Improved currency selection.
//#define PROGRAM_VERSION "v0 7" // Improvements to logging, notifications, investigation into Windows based check for EA running. 
...
extern bool    GLOBAL_testing                = true;           //Makes the EA fire on increment, not timeframe. Resets log, disables notifications.
...
int init(){  
   ...
   GLOBAL_dumpsumarray_file      = AccountNumber()+"-sumarraydump.csv";
...
void account_update(){
   ...
   if(ACCOUNT_previousbalance > 0){
      FUNCVAR_accountchangepercentage = (1 - (AccountBalance() / ACCOUNT_previousbalance+0.0000000000001))*100;
   }else{
...
void signature_createsumarray(int FUNCGET_barnumber = 0){
   ...
   for(FUNCVAR_basecurrency = 0; FUNCVAR_basecurrency < ArraySize(CURRENCY_entities); FUNCVAR_basecurrency++){
      for(FUNCVAR_tradedcurrency = 0; FUNCVAR_tradedcurrency < ArraySize(CURRENCY_entities); FUNCVAR_tradedcurrency++){
         FUNCVAR_currentpair = CURRENCY_entities[FUNCVAR_basecurrency] + CURRENCY_entities[FUNCVAR_tradedcurrency];
         if(
            CURRENCY_entities[FUNCVAR_basecurrency] != CURRENCY_entities[FUNCVAR_tradedcurrency] &&
...
double getinfo(int FUNCGET_what, string FUNCGET_pair, int FUNCGET_timeframe, int FUNCGET_barnumber) {
   ...
   if(iTime(FUNCGET_pair, FUNCGET_timeframe, FUNCGET_barnumber) == 0){
      log("Trying to get data beyond limit");
   }
...
void init_populatecurrencyarray(){
   function_start("init_populatecurrencyarray");
...
int init_getmaxlookback(){
   ...
   for(int a=0;a<CURRENCY_numberofpairs;a++){
      ...
      while(
         ...
         iTime(FUNCVAR_currentpair, GLOBAL_minitimeframe, FUNCVAR_barnumber) > (TimeCurrent() - GLOBAL_lookback)
      ){
         FUNCVAR_barnumber++;
      }  
      if(iTime(FUNCVAR_currentpair, GLOBAL_minitimeframe, FUNCVAR_barnumber) == 0){
         FUNCVAR_newlookback = iTime(FUNCVAR_currentpair, GLOBAL_timeframe, 0) - iTime(FUNCVAR_currentpair, GLOBAL_timeframe, iBarShift(FUNCVAR_currentpair, GLOBAL_timeframe, iTime(FUNCVAR_currentpair, GLOBAL_minitimeframe, FUNCVAR_barnumber-1)) - 1);
         log("GLOBAL_lookback changed from "+GLOBAL_lookback+" to "+FUNCVAR_newlookback+" "+FUNCVAR_currentpair+" back to barnumber "+(FUNCVAR_barnumber-1)+" dated: "+humandate(iTime(FUNCVAR_currentpair, GLOBAL_minitimeframe, FUNCVAR_barnumber-1)));
...
void init_resetfiles(){
   function_start("init_resetfiles");
   ...
   string FUNCVAR_sumheaderarray[14] = {"Date", "Date", "Day", "Pair", "Spread", "Signature", "Target", "Target Distance", "Target Positive", "Target Negative", "Order", "Result", "Rating", "Cases", "NonInverse", "Pair", "Recent", "Win"};
   FUNCVAR_file = openafile(GLOBAL_dumpsumarray_file, FUNCVAR_sumheaderarray);
   FileClose(FUNCVAR_file);
