...
bool signature_testsignrecordhigh(int FUNCGET_recordnumber){
   ...
   if(StringLen(SIGN_pair[FUNCGET_recordnumber]) > 0){
      if(
         SIGN_highbreakaverage[FUNCGET_recordnumber] / 4 > SIGN_target[FUNCGET_recordnumber] &&
...
bool signature_testsignrecordlow(int FUNCGET_recordnumber){
   ...
   if(StringLen(SIGN_pair[FUNCGET_recordnumber]) > 0){
      if(
         SIGN_lowbreakaverage[FUNCGET_recordnumber] / 4 > SIGN_target[FUNCGET_recordnumber] &&
         signature_testsignrecord(FUNCGET_recordnumber) == true
      ){
         function_end();
         return(true);
   ...
   for(FUNCVAR_counter=0;FUNCVAR_counter<ORDERS_currentnumberoforders;FUNCVAR_counter++){
      if(ORDERS_cmd[FUNCVAR_counter] > 0){
         ...
         while(FUNCVAR_ticket <= 0 && FUNCVAR_attempt < 6){
            ...
            FUNCVAR_ticket = OrderSend(ORDERS_pair[FUNCVAR_counter], ORDERS_cmd[FUNCVAR_counter], ORDERS_volume[FUNCVAR_counter], ORDERS_price[FUNCVAR_counter], 2, ORDERS_stoploss[FUNCVAR_counter], ORDERS_takeprofit[FUNCVAR_counter], "", 0, iTime(Symbol(), GLOBAL_timeframe, 0)+60*60*24);
            if(FUNCVAR_ticket <= 0){
...
double getinfo(int FUNCGET_what, string FUNCGET_pair, int FUNCGET_timeframe, int FUNCGET_barnumber) {
   ...
   switch (FUNCGET_what){
      ...
      case 504: //Get target for SL 
         FUNCVAR_return = MathMax(getinfo(502, FUNCGET_pair, GLOBAL_timeframe, FUNCGET_barnumber) * 6, MarketInfo(FUNCGET_pair, MODE_STOPLEVEL)*1.1 * MarketInfo(FUNCGET_pair, MODE_POINT));
         //target has two minimums. 1) 5 times spread (can change to as small as 2.5 due to spread change) and 2) stoploss minimum distance * 1.1
         log((getinfo(502, FUNCGET_pair, GLOBAL_timeframe, FUNCGET_barnumber) * 5)+" "+(MarketInfo(FUNCGET_pair, MODE_STOPLEVEL)*1.1 * MarketInfo(FUNCGET_pair, MODE_POINT))+" "+FUNCVAR_return );
         function_end();
         return(FUNCVAR_return);
         break;
      default:
