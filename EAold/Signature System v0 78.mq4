       ...
       for(FUNCVAR_barnumberloop=FUNCGET_barnumber + 2;FUNCVAR_barnumberloop < CURRENCY_allpairslookback[FUNCVAR_currencycounter]-2;FUNCVAR_barnumberloop++){ //Start looking at data 2 bars back from the bar we're looking at and 2 bars before the end of the data
         ...
         if(
            ...
            for(FUNCVAR_targetcounter=0;FUNCVAR_targetcounter < ArraySize(GLOBAL_targetpercentages); FUNCVAR_targetcounter++){
               ...
               SIGNATURE_NEXTBAR_close[FUNCVAR_targetcounter] = SIGNATURE_NEXTBAR_close[FUNCVAR_targetcounter] + ( ( getinfo(4, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) - getinfo(1, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) ) / getinfo(7, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop ) );
...
string signature_getbarsignature(string FUNCGET_pair, int FUNCGET_timeframe, int FUNCGET_barnumber){
   ...
   FUNCVAR_signature = FUNCVAR_signature + TimeDayOfWeek(getinfo(5, FUNCGET_pair, FUNCGET_timeframe, FUNCGET_barnumber)) + "D";
   
   if(getinfo(4, FUNCGET_pair, FUNCGET_timeframe, FUNCGET_barnumber) < iMA(FUNCGET_pair, FUNCGET_timeframe, 5, FUNCGET_barnumber, MODE_SMA, PRICE_LOW, 0)){
      FUNCVAR_signature = FUNCVAR_signature + "UMA";
   }else if(getinfo(4, FUNCGET_pair, FUNCGET_timeframe, FUNCGET_barnumber) > iMA(FUNCGET_pair, FUNCGET_timeframe, 5, FUNCGET_barnumber, MODE_SMA, PRICE_HIGH, 0)){
      FUNCVAR_signature = FUNCVAR_signature + "AMA";
   }else{
      FUNCVAR_signature = FUNCVAR_signature + "IMA";
   }
...
void order_addsignaturetoorder(int FUNCGET_target, string FUNCGET_pair){
   ...
   if(ORDERS_recent_rating[ORDERS_currentnumberoforders] > 0){
      ORDERS_cmd[ORDERS_currentnumberoforders] = 0; // Buy
      ORDERS_currentnumberoforders++;
   }else if(ORDERS_recent_rating[ORDERS_currentnumberoforders] < 0){
...
void testing_runtests(){
   ...
   //Returns a report on the trades which would have been taken in the last number of days
      testing_outputdemonumberofdays(550);
