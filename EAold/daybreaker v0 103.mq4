...
extern string  CURRENCY_skippairs            = "USDCAD;CHFJPY;EURGBP;EURCAD;EURAUD;GBPUSD;GBPCHF;GBPCAD;GBPJPY;CADJPY"; //= "CHFSGD; EURSGD; EURDKK; EURNOK; EURSEK; USDTRY; EURTRY; GBPSGD; GBPSEK; GBPNOK; GBPDKK; GBPTRY; CADSGD; CADSEK; CADNOK; CADDKK; CADTRY; AUDSGD; NZDCHF; NZDSGD; NZDCAD; NOKJPY; HKDJPY; NOKSEK; SEKJPY";
...
void volatility_createvolarray(int FUNCGET_bar = 1){
   ...
   for(FUNCVAR_currencycounter=0;FUNCVAR_currencycounter<CURRENCY_selected_numberofpairs;FUNCVAR_currencycounter++){
      ...
      if(VOL_vol_exposedhighcount[FUNCVAR_currencycounter] != 0){
         //VOL_vol_hightp[FUNCVAR_currencycounter] = (VOL_vol_exposedhigh[FUNCVAR_currencycounter]*VOLCONST_vol_targetsize / (VOL_vol_exposedhighcount[FUNCVAR_currencycounter]*1.0)) * ( 1.0) * ( (VOLCONST_vol_targetfactor)*1.0  / (VOLCONST_vol_targetfactor+1.0) );
         //VOL_vol_highsl[FUNCVAR_currencycounter] = (VOL_vol_exposedhigh[FUNCVAR_currencycounter]*VOLCONST_vol_targetsize / (VOL_vol_exposedhighcount[FUNCVAR_currencycounter]*1.0)) * (-1.0) * ( (             1.0         )      / (VOLCONST_vol_targetfactor+1.0) );
         VOL_vol_highsl[FUNCVAR_currencycounter] = (-1.0) * MathAbs(VOL_vol_spread[FUNCVAR_currencycounter]  * VOLCONST_vol_slfactor);
         VOL_vol_hightp[FUNCVAR_currencycounter] = ( 1.0) * MathAbs(VOL_vol_highsl[FUNCVAR_currencycounter]  * VOLCONST_vol_targetfactor);
      }
      ...
      if(VOL_vol_exposedlowcount[FUNCVAR_currencycounter] != 0){
         //VOL_vol_lowtp[FUNCVAR_currencycounter] = (VOL_vol_exposedlow[FUNCVAR_currencycounter]*VOLCONST_vol_targetsize / (VOL_vol_exposedlowcount[FUNCVAR_currencycounter]*1.0)) * ( 1.0) * ( (VOLCONST_vol_targetfactor)*1.0  / (VOLCONST_vol_targetfactor+1.0) );
         //VOL_vol_lowsl[FUNCVAR_currencycounter] = (VOL_vol_exposedlow[FUNCVAR_currencycounter]*VOLCONST_vol_targetsize / (VOL_vol_exposedlowcount[FUNCVAR_currencycounter]*1.0)) * (-1.0) * ( (             1.0         )      / (VOLCONST_vol_targetfactor+1.0) );
         VOL_vol_lowsl[FUNCVAR_currencycounter] = (-1.0) * MathAbs(VOL_vol_spread[FUNCVAR_currencycounter]  * VOLCONST_vol_slfactor);
         VOL_vol_lowtp[FUNCVAR_currencycounter] = ( 1.0) * MathAbs(VOL_vol_lowsl[FUNCVAR_currencycounter]   * VOLCONST_vol_targetfactor);
      }
...
void volatility_testvolarray(){
   ...
   for(int a=0;a<100;a++){
      if(StringLen(VOL_vol_pair[a]) > 2 ){
         ...
         VOL_vol_highpoint[a] = getinfo(2, VOL_vol_pair[a], VOLCONST_vol_timeframe, VOL_vol_bar[a]);
         VOL_vol_hightppoint[a] = VOL_vol_highpoint[a] + VOL_vol_hightp[a];
         VOL_vol_highslpoint[a] = VOL_vol_highpoint[a] + VOL_vol_highsl[a];  

         VOL_vol_lowpoint[a] = getinfo(3, VOL_vol_pair[a], VOLCONST_vol_timeframe, VOL_vol_bar[a]);
         VOL_vol_lowtppoint[a] = VOL_vol_lowpoint[a] - VOL_vol_lowtp[a];
         VOL_vol_lowslpoint[a] = VOL_vol_lowpoint[a] - VOL_vol_lowsl[a];
   
            FUNCVAR_minibar = iBarShift(VOL_vol_pair[a], VOLCONST_vol_minitimeframe, getinfo(5, VOL_vol_pair[a], VOLCONST_vol_timeframe, VOL_vol_bar[a] - 1));
            VOL_vol_highhit[a] = 0;
