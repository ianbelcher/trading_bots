...
extern int     GLOBAL_recentlookback         = 7500000;        //In Seconds. For the RECENT sub-signature, this is the lookback. 2.5M is about one month.
extern double  GLOBAL_stoplossfactor         = 3;              //The factor of size of the SL compared to the TP point for a trade.
...
extern double  GLOBAL_goalminrating          = 0.15;           //The minimum expected ROI on a trade 
extern double  GLOBAL_goalspreadfactor       = 4;              //Minimum target needs to be this many times the spread to negate its effects.
...
extern bool    CURRENCY_TRY                  = false; //= true;
...
double      GLOBAL_targetpercentages[10] = {0.8, 0.7, 0.6, 0.5, 0.45, 0.4, 0.35, 0.3, 0.25, 0.2};
...
bool        GLOBAL_isdemo;
bool        GLOBAL_dumpsumarraytitles = false;
...
int         CURRENCY_numberofpairs;
int         CURRENCY_numberofallpairs;
int         CURRENCY_allpairslookback[100];
string      CURRENCY_usedentities[100];
string      CURRENCY_allentities[100];
string      CURRENCY_pairs[100];
string      CURRENCY_allpairs[100];
...
void signature_cleararray(){
   ...
   for(int FUNCVAR_targetcounter=0;FUNCVAR_targetcounter<ArraySize(GLOBAL_targetpercentages);FUNCVAR_targetcounter++){
      SIGNATURE_signature[FUNCVAR_targetcounter] = "";     
...
void signature_dumpsignaturearray(string FUNCGET_signature, string FUNCGET_pair, int FUNCGET_date){
...
void signature_setsignaturearray(string FUNCGET_signature, string FUNCGET_pair, int FUNCGET_barnumber = 0){
   ...
   for(FUNCVAR_currencycounter=0;FUNCVAR_currencycounter<CURRENCY_numberofpairs;FUNCVAR_currencycounter++){
      FUNCVAR_currentpair = CURRENCY_allpairs[FUNCVAR_currencycounter];
       for(FUNCVAR_barnumberloop=FUNCGET_barnumber + 2;FUNCVAR_barnumberloop < CURRENCY_allpairslookback[FUNCVAR_currencycounter]-2;FUNCVAR_barnumberloop++){ //Start looking at data 2 bars back from the bar we're looking at and 2 bars before the end of the data
         ...
         if(
            ...
            for(FUNCVAR_targetcounter=0;FUNCVAR_targetcounter < ArraySize(GLOBAL_targetpercentages); FUNCVAR_targetcounter++){
               ...
               FUNCVAR_rangeofbar = getinfo(7, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop);
               if(FUNCVAR_rangeofbar > 0){
                  FUNCVAR_pointtarget = FUNCVAR_rangeofbar * GLOBAL_targetpercentages[FUNCVAR_targetcounter];
                  ...
                  if(FUNCVAR_signature == FUNCGET_signature){
                     ...
                     //log(humandate(getinfo(5, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1))+" "+SIGNATURE_signature[FUNCVAR_targetcounter]+" "+FUNCVAR_currentpair+" "+FUNCVAR_targetcounter+" "+FUNCVAR_return+" winner of "+FUNCVAR_positive+" "+FUNCVAR_positivehit+" & "+FUNCVAR_negative+" "+FUNCVAR_negativehit+" Now: "+SIGNATURE_NONINVERSE_result[FUNCVAR_targetcounter]);
                  }
                  SIGNATURE_TOTAL_cases[FUNCVAR_targetcounter]++;
                  SIGNATURE_TOTAL_result[FUNCVAR_targetcounter] = SIGNATURE_TOTAL_result[FUNCVAR_targetcounter] + FUNCVAR_return;

                  SIGNATURE_NEXTBAR_high[FUNCVAR_targetcounter] = SIGNATURE_NEXTBAR_high[FUNCVAR_targetcounter]   + ( ( getinfo(2, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) - getinfo(1, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) ) / FUNCVAR_rangeofbar );
                  SIGNATURE_NEXTBAR_low[FUNCVAR_targetcounter] = SIGNATURE_NEXTBAR_low[FUNCVAR_targetcounter]     + ( ( getinfo(3, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) - getinfo(1, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) ) / FUNCVAR_rangeofbar );
                  SIGNATURE_NEXTBAR_close[FUNCVAR_targetcounter] = SIGNATURE_NEXTBAR_close[FUNCVAR_targetcounter] + ( ( getinfo(4, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) - getinfo(1, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) ) / FUNCVAR_rangeofbar );
...
int signature_testsignaturearray(string FUNCGET_pair){
   ...
   for(FUNCVAR_targetcounter=0;FUNCVAR_targetcounter < ArraySize(GLOBAL_targetpercentages); FUNCVAR_targetcounter++){
      if(SIGNATURE_NONINVERSE_cases[FUNCVAR_targetcounter] != 0){
         FUNCVAR_currentrating = (SIGNATURE_NONINVERSE_result[FUNCVAR_targetcounter]+0.0) / (SIGNATURE_NONINVERSE_cases[FUNCVAR_targetcounter]+0.0);
      }else{
         FUNCVAR_currentrating = 0;
      }   
      ...
      if( //Decide if signature can be added to sum array
         ...
         SIGNATURE_NONINVERSE_cases[FUNCVAR_targetcounter] >= GLOBAL_goalminnumberofcases &&
...
string signature_getbarsignature(string FUNCGET_pair, int FUNCGET_timeframe, int FUNCGET_barnumber){
   ...
   FUNCVAR_signature = FUNCVAR_signature + TimeDayOfWeek(iTime(FUNCGET_pair, FUNCGET_timeframe, FUNCGET_barnumber));
...
void order_addsignaturetoorder(int FUNCGET_target, string FUNCGET_pair){
   function_start("signature_addsignaturetoorder", true);
      
   ORDERS_pair[ORDERS_currentnumberoforders] = FUNCGET_pair;
   ORDERS_signature[ORDERS_currentnumberoforders] = SIGNATURE_signature[FUNCGET_target];
   ORDERS_target[ORDERS_currentnumberoforders] = GLOBAL_targetpercentages[FUNCGET_target];
   ORDERS_tested_targetdistance[ORDERS_currentnumberoforders] = getinfo(7, FUNCGET_pair, GLOBAL_timeframe, ORDERS_barnumber) * GLOBAL_targetpercentages[FUNCGET_target];
   ORDERS_spread[ORDERS_currentnumberoforders] = getinfo(502, FUNCGET_pair, GLOBAL_timeframe, ORDERS_barnumber);
   ...
   if(SIGNATURE_RECENT_cases[FUNCGET_target] > 0){
      ...
      ORDERS_recent_result[ORDERS_currentnumberoforders] = SIGNATURE_RECENT_result[FUNCGET_target];
   ...
   if(SIGNATURE_TOTAL_cases[FUNCGET_target] > 0){
      ...
      ORDERS_total_result[ORDERS_currentnumberoforders] = SIGNATURE_TOTAL_result[FUNCGET_target];
   ...
   if(SIGNATURE_NONINVERSE_cases[FUNCGET_target] > 0){
      ...
      ORDERS_noninverse_result[ORDERS_currentnumberoforders] = SIGNATURE_NONINVERSE_result[FUNCGET_target];
   }
     
   if(ORDERS_noninverse_rating[ORDERS_currentnumberoforders] > 0){
      ORDERS_cmd[ORDERS_currentnumberoforders] = 0; // Buy
      ORDERS_currentnumberoforders++;
   }else if(ORDERS_noninverse_rating[ORDERS_currentnumberoforders] < 0){
...
void order_addresultstoorder(){
   ...
   for(FUNCVAR_counter=0; FUNCVAR_counter < ORDERS_currentnumberoforders; FUNCVAR_counter++){
      if(ORDERS_cmd[FUNCVAR_counter] >=0){
         ...
         ORDERS_tested_endprice[FUNCVAR_counter] = getinfo(1, ORDERS_pair[FUNCVAR_counter], GLOBAL_timeframe, FUNCVAR_barnumber - 2);
...
void order_createorderarray(int FUNCGET_barnumber = 0){
   ...
   for(int a=0;a<CURRENCY_numberofpairs;a++){
      ...
      //signature_dumpsignaturearray(FUNCVAR_signature, FUNCVAR_currentpair, ORDERS_date);
      FUNCVAR_targetindex = signature_testsignaturearray(FUNCVAR_currentpair);
      if(FUNCVAR_targetindex >= 0){
         order_addsignaturetoorder(FUNCVAR_targetindex, FUNCVAR_currentpair);
      }
   }
...
int init(){  
   ...
   GLOBAL_cronincrement          = MINUTE;      //In Minutes. What time frame to run the smallest cron task
...
int init_getlookback(){
   ...
   for(int a=0;a<CURRENCY_numberofallpairs;a++){
      ...
      CURRENCY_allpairslookback[a] = MathMin(CURRENCY_allpairslookback[a], FUNCVAR_barnumber);
      log(FUNCVAR_currentpair+" back to barnumber "+CURRENCY_allpairslookback[a]+" "+FUNCVAR_barnumber+" dated: "+humandate(iTime(FUNCVAR_currentpair, GLOBAL_timeframe, CURRENCY_allpairslookback[a])));
   }
