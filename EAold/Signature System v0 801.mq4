...
#define PROGRAM_VERSION "v0 801" //Hourly bartype betting
...
int signature_testsignaturearray(string FUNCGET_pair){
   ...
   if(GLOBAL_incminibar == true){
      ...
      if(SIGN_NEXTBAR_countedbars > 0){
         FUNCVAR_closedigital = (SIGN_NEXTBAR_close+0.0) / (SIGN_NEXTBAR_countedbars+0.0);
         if(FUNCVAR_closedigital > 0.1){
            SIGN_cmd = 1; // Sell which is against the average...
         }else if(FUNCVAR_closedigital < -0.1){
            SIGN_cmd = 0; // Buy which is against the average...
         }else{
            SIGN_cmd = -1;
         }
      }
...
void order_addsignaturetoorder(int FUNCGET_target){
   ...
   if(SIGN_NEXTBAR_countedbars > 0){
      ORDERS_nextbar_high[ORDERS_currentnumberoforders] = (SIGN_NEXTBAR_high+0.0) / (SIGN_NEXTBAR_countedbars+0.0);
      ORDERS_nextbar_low [ORDERS_currentnumberoforders] = (SIGN_NEXTBAR_low+0.0) / (SIGN_NEXTBAR_countedbars+0.0);
      ORDERS_nextbar_close[ORDERS_currentnumberoforders] = (SIGN_NEXTBAR_close+0.0) / (SIGN_NEXTBAR_countedbars+0.0);
      ORDERS_nextbar_closedigital[ORDERS_currentnumberoforders] = (SIGN_NEXTBAR_closedigital+0.0) / (SIGN_NEXTBAR_countedbars+0.0);
   }
...
int init(){  
   ...
   GLOBAL_minitimeframe          = MINUTE * 5;     //In Minutes. The timeframe for judging success of a signature, must be less than the main timeframe.
   GLOBAL_cronincrement          = MINUTE * 1;     //In Minutes. What time frame to run the smallest cron task
...
void testing_showorders(int FUNCGET_periods = 1, int FUNCGET_startbar = 1){
