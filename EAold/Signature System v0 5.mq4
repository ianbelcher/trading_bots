...
//#define PROGRAM_VERSION "v0 6" // Improvements to logging, notifications, investigation into Windows based check for EA running. 
...
void init_populatecurrencyarray(){
   ...
   if(CURRENCY_USD){
      GLOBAL_entities[0] = "USD";
   }   
   if(CURRENCY_CHF){
      GLOBAL_entities[1] = "CHF";
   }   
   if(CURRENCY_EUR){
      GLOBAL_entities[2] = "EUR";
   }   
   if(CURRENCY_GBP){
      GLOBAL_entities[3] = "GBP";
   }
   if(CURRENCY_CAD){
      GLOBAL_entities[4] = "CAD";
   }   
   if(CURRENCY_JPY){
      GLOBAL_entities[5] = "JPY";
   }   
   if(CURRENCY_AUD){
      GLOBAL_entities[6] = "AUD";
   }   
   if(CURRENCY_NZD){
      GLOBAL_entities[7] = "NZD";
   }
   if(CURRENCY_SGD){
      GLOBAL_entities[8] = "SGD";
   }   
   if(CURRENCY_HKD){
      GLOBAL_entities[9] = "HKD";
   }   
   if(CURRENCY_DKK){
      GLOBAL_entities[10] = "DKK";
   }   
   if(CURRENCY_NOK){
      GLOBAL_entities[11] = "NOK";
   }
   if(CURRENCY_SEK){
      GLOBAL_entities[12] = "SEK";
   }   
   if(CURRENCY_TRY){
      GLOBAL_entities[13] = "TRY";
   }   
   if(CURRENCY_PLN){
      GLOBAL_entities[14] = "PLN";
   }   
   if(CURRENCY_MXN){
      GLOBAL_entities[15] = "MXN";
   }
   if(CURRENCY_XAU){
      GLOBAL_entities[16] = "XAU";
   }   
   if(CURRENCY_XAG){
      GLOBAL_entities[17] = "XAG";
   }      
   
   for(int a=0; a<18; a++){
      log(GLOBAL_entities[a]);
   }
   //GLOBAL_entities[0] = {"USD", "CHF", "EUR", "GBP", "CAD", "JPY", "AUD", "NZD", "SGD", "HKD", "DKK", "NOK", "SEK", "TRY", "PLN", "MXN", "XAU", "XAG" };
      ...
      for(FUNCVAR_tradedcurrency = 0; FUNCVAR_tradedcurrency < ArraySize(GLOBAL_entities); FUNCVAR_tradedcurrency++){
         ...
         if(
            ...
            if(iTime(FUNCVAR_currentpair, GLOBAL_minitimeframe, FUNCVAR_barnumber) == 0){
               log(FUNCVAR_currentpair+" back to barnumber "+(FUNCVAR_barnumber-1)+" dated: "+humandate(iTime(FUNCVAR_currentpair, GLOBAL_minitimeframe, FUNCVAR_barnumber-1)));
               ...
               log("GLOBAL_lookback changed from "+GLOBAL_lookback+" to "+FUNCVAR_newlookback);
