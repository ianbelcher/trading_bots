...
double GLOBAL_targetpercentages[8] = {0.764, 0.618, 0.50, 0.382, 0.33, 0.236, 0.15};
//double GLOBAL_targetpercentages[1] = {0.25};
...
//string GLOBAL_entities[8] = {"USD", "CHF", "EUR", "GBP", "CAD", "JPY", "AUD", "NZD" };
...
int
   ...
   GLOBAL_goalminnumberofcases,
   GLOBAL_goalspreadfactor,
   GLOBAL_processingtime,
   GLOBAL_timetillnextbar,
   GLOBAL_timeofnextbar,
...
double
   GLOBAL_goalminrating,
   GLOBAL_riskpertradepercentage,
...
int init(){
   ...
   Alert("Running Live - " + RUNNINGLIVE);
   ...
   GLOBAL_goalminnumberofcases   = 6;              // There needs to be at least this many cases of a previous signature for it to be counted.
   GLOBAL_goalminrating          = 0.34;           // 0.01 over 0.33 to clear 0.33 out which is a popular spot for lower cased signatures.
   GLOBAL_goalspreadfactor       = 5;              // Minimum target needs to be this many times the spread to negate its effects.
   GLOBAL_processingtime         = 60 * MINUTE*5;  // Inital processing time starts this long before the next bar. 
   GLOBAL_timetillnextbar        = iTime(Symbol(), GLOBAL_timeframe, 0) * 2 - iTime(Symbol(), GLOBAL_timeframe, 1) - TimeCurrent();
   GLOBAL_riskpertradepercentage = 0.01;           // The percentage of the account to risk per trade. 
   ...
   GLOBAL_pausetime              = 200;            // In milliseconds. Time to wait when server needs time.
...
void order_createorderarray(){
   ...
   for(a=0;a<GLOBAL_currentnumberoforders;a++){
      ...
      ORDERS_lotshare[a] = SIGNATURE_SUM_score[FUNCVAR_introunder] / SIGNATURE_SUM_totalscore;
   }
}
...
void order_send(){
   ...
   for(FUNCVAR_count=0;FUNCVAR_count<GLOBAL_currentnumberoforders;FUNCVAR_count++){
      ...
      if(ORDERS_order[FUNCVAR_count] == "OP_SELL"){
         ...
         FUNCVAR_stoploss = FUNCVAR_price - FUNCVAR_target;
         FUNCVAR_takeprofit = FUNCVAR_price + ORDERS_target[FUNCVAR_count] * getinfo(7, FUNCVAR_symbol, GLOBAL_timeframe, 0);
      }
      FUNCVAR_attempt = 1;
...
double order_getlotsize(double FUNCGET_target, string FUNCGET_currency){
   ...
   FUNCVAR_targetinticks = FUNCGET_target/MarketInfo(FUNCGET_currency, MODE_TICKSIZE);
...
void order_closeout(){
   ...
   for(FUNCVAR_count=0; FUNCVAR_count < FUNCVAR_numberoforders; FUNCVAR_count++) {
      ...
      if(OrderType() == OP_SELL){
         ...
         while(FUNCVAR_ticket == FALSE && FUNCVAR_attempt < 6){
            FUNCVAR_errornumber = GetLastError();
            log("Closing Order failed attempt "+FUNCVAR_attempt+": "+OrderTicket()+" "+OrderLots()+" "+MarketInfo(OrderSymbol(), MODE_BID)+" "+"200");
            log("Closing Order failed with error #"+FUNCVAR_errornumber+" - "+ErrorDescription(FUNCVAR_errornumber));
            Sleep(GLOBAL_pausetime*5);
            FUNCVAR_attempt++;
            RefreshRates();
...
double getinfo(int FUNCGET_what, string FUNCGET_pair, int FUNCGET_timeframe, int FUNCGET_barnumber) {
   ...
   switch (FUNCGET_what){
      ...
      default:
         ...
         if(RUNNINGLIVE == "true"){
            Alert("***INCORRECT Selection made in switch for function getinfo*** Looking for " + FUNCGET_what);
         }         
         return(0);
         break;
   }
...
void log(string FUNCGET_msg = "NULL"){
   int 
      FUNCVAR_logfile
      ;
   if(RUNNINGLIVE != "true" && FUNCGET_msg == "NULL"){
      FileDelete("log.csv");
      GetLastError();
   }
