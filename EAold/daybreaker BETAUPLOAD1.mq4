...
#define PROGRAM_VERSION "v0 100" //Inital Program Writing
...
bool signature_testsignrecordhigh(int FUNCGET_recordnumber){
   ...
   if(StringLen(SIGN_pair[FUNCGET_recordnumber]) > 0){
      if(SIGN_highbreakaverage[FUNCGET_recordnumber] > getinfo(504, SIGN_pair[FUNCGET_recordnumber], GLOBAL_timeframe, 0) * GLOBAL_takeprofitfactor){
         function_end();
         return(true);
      }
   }
...
bool signature_testsignrecordlow(int FUNCGET_recordnumber){
   ...
   if(StringLen(SIGN_pair[FUNCGET_recordnumber]) > 0){
      if(SIGN_lowbreakaverage[FUNCGET_recordnumber] > getinfo(504, SIGN_pair[FUNCGET_recordnumber], GLOBAL_timeframe, 0) * GLOBAL_takeprofitfactor){
         function_end();
         return(true);
      }
   }   
...
void order_createorderarray(int FUNCGET_barnumber = 0){
   ...
   signature_cleararray();
   signature_createarray(FUNCGET_barnumber);   
   ...
   for(FUNCVAR_counter=0;FUNCVAR_counter<200;FUNCVAR_counter++){
      if(signature_testsignrecordhigh(FUNCVAR_counter) == true){
         ...
         ORDERS_target[ORDERS_currentnumberoforders] = getinfo(504, ORDERS_pair[ORDERS_currentnumberoforders], GLOBAL_timeframe, FUNCGET_barnumber);
   ...
   for(FUNCVAR_counter=0;FUNCVAR_counter<ORDERS_currentnumberoforders;FUNCVAR_counter++){
      if(ORDERS_cmd[FUNCVAR_counter] > 0){
         ...
         while(FUNCVAR_ticket < 0 && FUNCVAR_attempt < 6){
            ...
            ORDERS_target[FUNCVAR_counter] = getinfo(504, ORDERS_pair[FUNCVAR_counter], GLOBAL_timeframe, 0);
            ...
            FUNCVAR_ticket = OrderSend(ORDERS_pair[FUNCVAR_counter], ORDERS_cmd[FUNCVAR_counter], ORDERS_volume[FUNCVAR_counter], ORDERS_price[FUNCVAR_counter], 2, ORDERS_stoploss[FUNCVAR_counter], ORDERS_takeprofit[FUNCVAR_counter], "", 0, Time[0]+60*60*24);
            if(FUNCVAR_ticket < 0){
...
double getinfo(int FUNCGET_what, string FUNCGET_pair, int FUNCGET_timeframe, int FUNCGET_barnumber) {
   ...
   switch (FUNCGET_what){
      ...
      case 503: //Minimum distance between market and SL point
         ...
         return(FUNCVAR_return);
         break;
      case 504: //Get target for SL and TP factor
         FUNCVAR_return = MarketInfo(FUNCGET_pair, MODE_STOPLEVEL)*1.1 * MarketInfo(FUNCGET_pair, MODE_POINT);
         if(FUNCVAR_return == 0){
            FUNCVAR_return = (iHigh(FUNCGET_pair, FUNCGET_timeframe, FUNCGET_barnumber + 1) - iLow(FUNCGET_pair, FUNCGET_timeframe, FUNCGET_barnumber + 1)) / 20;
         }
         function_end();
         return(FUNCVAR_return);
         break;
