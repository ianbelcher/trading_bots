...
int
   ...
   GLOBAL_tempfile // DELETE THIS LATER
...
int init(){  
   ...
   GLOBAL_tempfile = FileOpen("Alltargets.csv", FILE_CSV|FILE_WRITE|FILE_READ );
...
void signature_createsumarray(int FUNCGET_barnumber = 0){
   ...
   for(int a=0;a<CURRENCY_numberofpairs;a++){
      ...
      signature_dumpsignaturearray(FUNCVAR_signature, FUNCVAR_currentpair, SIGNATURE_SUM_date);
...
void testing_runtests(){
   ...
   //Returns a report on the trades which would have been taken in the last number of days
      testing_outputdemonumberofdays(330);
