...
#define PROGRAM_VERSION "hour break v0 101"
...
extern bool    GLOBAL_runtests               = false;          //This fires the testing class which stops the EA after initialisation
...
BUY_STOP at the high of the previous hour
SELL_STOP at the low of the previous hour
...
Current issues which are/can/may effect performance of this strategy.
   ...
   Currently looking at 1 minute bars to decipher winners. There could be the case where many entries are stopped out as the bar after the entry is the first bar looked at.
   Due to dynamic spreads, stops may be hit more often than is testable.
...
void testing_createarray(int FUNCGET_bar = 1){
   ...
   for(FUNCVAR_currencycounter=0;FUNCVAR_currencycounter<CURRENCY_selected_numberofpairs;FUNCVAR_currencycounter++){
      ...
      TESTING_VAR_spread[FUNCVAR_currencycounter] = getinfo(502, TESTING_VAR_pair[FUNCVAR_currencycounter], VOLCONST_vol_timeframe, FUNCGET_bar + 1);
      ...
      TESTING_VAR_stoplossfactor[FUNCVAR_currencycounter] = getinfo(503, TESTING_VAR_pair[FUNCVAR_currencycounter], VOLCONST_vol_timeframe, FUNCGET_bar + 1);
      ...
      // DELETE THIS
      TESTING_VAR_spread[FUNCVAR_currencycounter] = TESTING_VAR_stoplossfactor[FUNCVAR_currencycounter] / 2;
      //
...
void testing_testarray(){
   ...
   for(int a=0;a<100;a++){
      if(StringLen(TESTING_VAR_pair[a]) > 2 ){
         ...
         TESTING_VAR_highpoint[a] = getinfo(2, TESTING_VAR_pair[a], VOLCONST_vol_timeframe, TESTING_VAR_bar[a] + 1);// - MathAbs(TESTING_VAR_sl[a]) * 0; // 1 spread inside the bar seems best atm
         ...
         TESTING_VAR_lowpoint[a] = getinfo(3, TESTING_VAR_pair[a], VOLCONST_vol_timeframe, TESTING_VAR_bar[a] + 1);// + MathAbs(TESTING_VAR_sl[a]) * 0; // 2 spreads outside the bar seems best atm
         ...
         TESTING_VAR_lowslpoint[a] = TESTING_VAR_lowpoint[a] + TESTING_VAR_sl[a];
            ...
            if(FUNCVAR_minibar > 0 && (TESTING_VAR_ratio_spread[a] >= VOLCONST_vol_minsprdrto && TESTING_VAR_ratio_slfactor[a] >= VOLCONST_vol_minslrto)){
               ...
               while(FUNCVAR_minibar - FUNCVAR_counter > 0 && FUNCVAR_counter < (VOLCONST_vol_timeframe / VOLCONST_vol_minitimeframe) - 1 && TESTING_VAR_highhit[a] == 0){
                  if(
                     TESTING_VAR_highpoint[a] <= getinfo(2, TESTING_VAR_pair[a], VOLCONST_vol_minitimeframe, FUNCVAR_minibar - FUNCVAR_counter) &&
                     TESTING_VAR_highpoint[a] >= getinfo(3, TESTING_VAR_pair[a], VOLCONST_vol_minitimeframe, FUNCVAR_minibar - FUNCVAR_counter)
                  ){
...
void account_createorders(){
   ...
   for(FUNCVAR_currencycounter=0;FUNCVAR_currencycounter<CURRENCY_selected_numberofpairs;FUNCVAR_currencycounter++){
      ...
      while(FUNCVAR_ticket < 0 && FUNCVAR_attempt < 6 && FUNCVAR_volume > 0){
         FUNCVAR_price = getinfo(2, FUNCVAR_symbol, GLOBAL_timeframe, 1);
      ...
      while(FUNCVAR_ticket < 0 && FUNCVAR_attempt < 6 && FUNCVAR_volume > 0){
         FUNCVAR_price = getinfo(3, FUNCVAR_symbol, GLOBAL_timeframe, 1);
         //FUNCVAR_sl = FUNCVAR_price + FUNCVAR_target;
         //FUNCVAR_tp = FUNCVAR_price - (FUNCVAR_target * 20);
         FUNCVAR_sl = FUNCVAR_price - FUNCVAR_target;
         FUNCVAR_tp = FUNCVAR_price + (FUNCVAR_target * 20);
