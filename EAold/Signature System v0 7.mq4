...
int init(){  
   ...
   GLOBAL_recentlookback         = 60 * MONTH * 3; //In Seconds. For the RECENT sub-signature, this is the lookback.
...
void cron_update(){
   ...
   if(iTime(Symbol(), PERIOD_W1, 0) * 2 - iTime(Symbol(), PERIOD_W1, 1) - TimeCurrent() < TIME_weekcrontime){
      cron_endweek();
   }
   ...
   if(iTime(Symbol(), PERIOD_D1, 0) * 2 - iTime(Symbol(), PERIOD_D1, 1) - TimeCurrent() < TIME_daycrontime){
      cron_endday();
   }
   ...
   if(iTime(Symbol(), PERIOD_H1, 0) * 2 - iTime(Symbol(), PERIOD_H1, 1) - TimeCurrent() < TIME_hourcrontime){
      cron_endhour();
   }
   ...
   if(iTime(Symbol(), GLOBAL_cronincrement, 0) * 2 - iTime(Symbol(), GLOBAL_cronincrement, 1) - TimeCurrent() < TIME_incrementcrontime){
      cron_endincrement();
   }
      ...
      for(int b=1;b<6;b++){
         for(int c=1;c<6;c++){
            for(int d=1;d<6;d++){
               ...
               log("SignatureOutput;"+FUNCVAR_signature+";"+SIGNATURE_TOTAL_cases[5]+";"+SIGNATURE_TOTAL_result[5]+";"+SIGNATURE_RECENT_cases[5]+";"+SIGNATURE_RECENT_result[5]); // Log the 5th target in the for this signature.
            }
         }
      }
   }
