//+------------------------------------------------------------------+
//|                                 Signature System v0 802 BETA.mq4 |
...
//#define PROGRAM_VERSION "v0 791" //Last good hourly version. Fading into obscurity...
//Investigating a simple hourly spreadbet. 
#define PROGRAM_VERSION "v0 802" //Hourly bartype betting
...
extern bool    GLOBAL_testing                = false;          //Makes the EA fire on increment, not timeframe. Resets log.
...
extern int     GLOBAL_maxlookback            = 75000000;       //In seconds. This is the max overall lookback for all pairs. 2.5M is about one month.
...
extern double  GLOBAL_stoplossfactor         = 1;              //The factor of size of the SL compared to the TP point for a trade.
extern int     GLOBAL_spreadfactor           = 20;             //The number of times of the spread to set the SL and TP points. 

extern bool    GLOBAL_incminibar             = false;          //This section turns off the RECENT NONINVERSE PAIR & TOTAL sections of the signature.
...
double      GLOBAL_targetpercentages[9] = {0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.25, 0.2, 0.15};
...
string      SIGN_pair;
int         SIGN_cmd;
string      SIGN_signature;
int         SIGN_barnumber;
int         SIGN_MINIBAR_RECENT_datemin;
int         SIGN_MINIBAR_RECENT_datemax;
int         SIGN_MINIBAR_RECENT_cases[15]; //Recent results
int         SIGN_MINIBAR_RECENT_result[15];
int         SIGN_MINIBAR_TOTAL_cases[15]; //All results
int         SIGN_MINIBAR_TOTAL_result[15];
int         SIGN_MINIBAR_NONINVERSE_cases[15]; //Without inverse results
int         SIGN_MINIBAR_NONINVERSE_result[15];
int         SIGN_MINIBAR_PAIR_cases[15];
int         SIGN_MINIBAR_PAIR_result[15];

double      SIGN_NEXTBAR_high;
double      SIGN_NEXTBAR_low;
double      SIGN_NEXTBAR_close;
double      SIGN_NEXTBAR_closedigital;
int         SIGN_NEXTBAR_countedbars;
...
void signature_cleararray(){
   ...
   SIGN_NEXTBAR_high = 0;
   SIGN_NEXTBAR_low = 0;
   SIGN_NEXTBAR_close = 0;
   SIGN_NEXTBAR_closedigital = 0;
   SIGN_NEXTBAR_countedbars = 0;

   for(int FUNCVAR_targetcounter=0;FUNCVAR_targetcounter < ArraySize(GLOBAL_targetpercentages);FUNCVAR_targetcounter++){   
      SIGN_MINIBAR_RECENT_cases[FUNCVAR_targetcounter] = 0;
      SIGN_MINIBAR_RECENT_result[FUNCVAR_targetcounter] = 0;
      SIGN_MINIBAR_TOTAL_cases[FUNCVAR_targetcounter] = 0;
      SIGN_MINIBAR_TOTAL_result[FUNCVAR_targetcounter] = 0;
      SIGN_MINIBAR_NONINVERSE_cases[FUNCVAR_targetcounter] = 0;
      SIGN_MINIBAR_NONINVERSE_result[FUNCVAR_targetcounter] = 0;
      SIGN_MINIBAR_PAIR_cases[FUNCVAR_targetcounter] = 0;
      SIGN_MINIBAR_PAIR_result[FUNCVAR_targetcounter] = 0;
...
void signature_dumpsignaturearray(){
   ...
   if(GLOBAL_incminibar == true){
      ...
      FUNCVAR_text =
         ...
         SIGN_MINIBAR_RECENT_cases[FUNCVAR_targetcounter]+";"+
         SIGN_MINIBAR_RECENT_result[FUNCVAR_targetcounter]+";"+
         SIGN_MINIBAR_TOTAL_cases[FUNCVAR_targetcounter]+";"+
         SIGN_MINIBAR_TOTAL_result[FUNCVAR_targetcounter]+";"+
         SIGN_MINIBAR_NONINVERSE_cases[FUNCVAR_targetcounter]+";"+
         SIGN_MINIBAR_NONINVERSE_result[FUNCVAR_targetcounter]+";"+
         SIGN_MINIBAR_PAIR_cases[FUNCVAR_targetcounter]+";"+
         SIGN_MINIBAR_PAIR_result[FUNCVAR_targetcounter]+";"+
         SIGN_NEXTBAR_high+";"+
         SIGN_NEXTBAR_low+";"+
         SIGN_NEXTBAR_close+";"+
         SIGN_NEXTBAR_closedigital+";"+
         SIGN_NEXTBAR_countedbars;
...
void signature_createsignaturearray(string FUNCGET_signature, string FUNCGET_pair, int FUNCGET_barnumber = 0){
   function_start("signature_createsignaturearray", true);
   ...
   SIGN_pair = FUNCGET_pair;
   SIGN_signature = FUNCGET_signature;
       ...
       for(FUNCVAR_barnumberloop=FUNCGET_barnumber + 2;FUNCVAR_barnumberloop < CURRENCY_selected_lookback[FUNCVAR_currencycounter]-2;FUNCVAR_barnumberloop++){ //Start looking at data 2 bars back from the bar we're looking at and 2 bars before the end of the data
         ...
         if(
            ...
            if(GLOBAL_incminibar == true){
               ...
               //v This section fills the array in which pertains to the minitimeframe.
                  ...
                  if(getinfo(5, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop) > getinfo(5, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCGET_barnumber) - GLOBAL_recentlookback){
                     SIGN_MINIBAR_RECENT_cases[FUNCVAR_targetcounter]++;
                     SIGN_MINIBAR_RECENT_result[FUNCVAR_targetcounter] = SIGN_MINIBAR_RECENT_result[FUNCVAR_targetcounter] + FUNCVAR_return;
                     if(FUNCVAR_signature == FUNCGET_signature){
                        SIGN_MINIBAR_NONINVERSE_cases[FUNCVAR_targetcounter]++;
                        SIGN_MINIBAR_NONINVERSE_result[FUNCVAR_targetcounter] = SIGN_MINIBAR_NONINVERSE_result[FUNCVAR_targetcounter] + FUNCVAR_return;
                     }
                  }
                              
                  SIGN_MINIBAR_TOTAL_cases[FUNCVAR_targetcounter]++;
                  SIGN_MINIBAR_TOTAL_result[FUNCVAR_targetcounter] = SIGN_MINIBAR_TOTAL_result[FUNCVAR_targetcounter] + FUNCVAR_return;

                  if(FUNCVAR_currentpair == FUNCGET_pair){
                     SIGN_MINIBAR_PAIR_cases[FUNCVAR_targetcounter]++;
                     SIGN_MINIBAR_PAIR_result[FUNCVAR_targetcounter] = SIGN_MINIBAR_PAIR_result[FUNCVAR_targetcounter] + FUNCVAR_return;
                  }
               } //End For target loop  
            } //End incminibar if
            //^ 
               
            //v This section fills the array which pertains to average OHLC positions
            
            FUNCVAR_range = getinfo(7, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop );
            if(FUNCVAR_range > 0){
               if(FUNCVAR_signature == FUNCGET_signature){
                  SIGN_NEXTBAR_high = SIGN_NEXTBAR_high   + ( ( getinfo(2, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) - getinfo(1, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) ) / FUNCVAR_range );
                  SIGN_NEXTBAR_low = SIGN_NEXTBAR_low     + ( ( getinfo(3, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) - getinfo(1, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) ) / FUNCVAR_range );
                  FUNCVAR_close = ( getinfo(4, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) - getinfo(1, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) ) / FUNCVAR_range;
                  FUNCVAR_closedigital = 0;
                  if(FUNCVAR_close > 0){FUNCVAR_closedigital = 1;}
                  if(FUNCVAR_close < 0){FUNCVAR_closedigital = -1;}
                  SIGN_NEXTBAR_close = SIGN_NEXTBAR_close + FUNCVAR_close;
                  SIGN_NEXTBAR_closedigital = SIGN_NEXTBAR_closedigital + FUNCVAR_closedigital;
                  SIGN_NEXTBAR_countedbars++;
               }else if(1==2){ // Switch inversion on/off for NEXTBAR stats
                  SIGN_NEXTBAR_high = SIGN_NEXTBAR_high   + ( ( getinfo(1, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) - getinfo(3, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) ) / FUNCVAR_range );
                  SIGN_NEXTBAR_low = SIGN_NEXTBAR_low     + ( ( getinfo(1, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) - getinfo(2, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) ) / FUNCVAR_range );
                  FUNCVAR_close = ( getinfo(1, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) - getinfo(4, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumberloop - 1) ) / FUNCVAR_range;
                  FUNCVAR_closedigital = 0;
                  if(FUNCVAR_close > 0){FUNCVAR_closedigital = 1;}
                  if(FUNCVAR_close < -0){FUNCVAR_closedigital = -1;}
                  SIGN_NEXTBAR_close = SIGN_NEXTBAR_close + FUNCVAR_close;
...
int signature_testsignaturearray(string FUNCGET_pair){
   ...
   if(GLOBAL_incminibar == true){
      ...
      for(FUNCVAR_targetcounter=0;FUNCVAR_targetcounter < ArraySize(GLOBAL_targetpercentages); FUNCVAR_targetcounter++){
         ...
         if(SIGN_MINIBAR_PAIR_cases[FUNCVAR_targetcounter] != 0){
            FUNCVAR_currentrating = (SIGN_MINIBAR_PAIR_result[FUNCVAR_targetcounter]+0.0) / (SIGN_MINIBAR_PAIR_cases[FUNCVAR_targetcounter]+0.0);
         }else{
            FUNCVAR_currentrating = 0;
         }   
         ...
         if( //Decide if signature can be added to sum array
            ...
            SIGN_MINIBAR_PAIR_cases[FUNCVAR_targetcounter] >= GLOBAL_goalminnumberofcases &&
            getinfo(7, FUNCGET_pair, GLOBAL_timeframe, SIGN_barnumber) * GLOBAL_targetpercentages[FUNCVAR_targetcounter] > GLOBAL_goalspreadfactor * getinfo(502, FUNCGET_pair, GLOBAL_timeframe, 1)
         ){
            FUNCVAR_bestrating = FUNCVAR_currentrating;
            FUNCVAR_besttarget = FUNCVAR_targetcounter;
         }
      }
      if(SIGN_MINIBAR_RECENT_result[FUNCVAR_besttarget] > 0){
         SIGN_cmd = 0; //Buy
      }else if(SIGN_MINIBAR_RECENT_result[FUNCVAR_besttarget] < 0){
         SIGN_cmd = 1; //Sell
      }else{
         SIGN_cmd = -1;
      }
      ...
      if(SIGN_NEXTBAR_countedbars > 0){
         FUNCVAR_closedigital = (SIGN_NEXTBAR_closedigital+0.0) /(SIGN_NEXTBAR_countedbars+0.0);
         if(FUNCVAR_closedigital > 0.1){
            SIGN_cmd = 0; //Buy
         }else if(FUNCVAR_closedigital < -0.1){
            SIGN_cmd = 1; //Sell
         }else{
            SIGN_cmd = -1;
         }
...
double      ORDERS_thisbar_high[100];
double      ORDERS_thisbar_low[100];
double      ORDERS_thisbar_close[100];
double      ORDERS_nextbar_high[100];
double      ORDERS_nextbar_low[100];
double      ORDERS_nextbar_close[100];
double      ORDERS_nextbar_highavg[100];
double      ORDERS_nextbar_lowavg[100];
double      ORDERS_nextbar_closeavg[100];
double      ORDERS_nextbar_closeavgdigital[100];

int         ORDERS_ticket[100];
double      ORDERS_openprice[100];
double      ORDERS_closeprice[100];
int         ORDERS_cmd[100];
...
string order_arrayrecordastext(int FUNCGET_recordnumber){
   ...
   if(GLOBAL_incminibar == true){
      FUNCVAR_text = 
         ...
         ORDERS_thisbar_high[FUNCGET_recordnumber]+";"+
         ORDERS_thisbar_low[FUNCGET_recordnumber]+";"+
         ORDERS_thisbar_close[FUNCGET_recordnumber]+";"+
         ORDERS_nextbar_high[FUNCGET_recordnumber]+";"+
         ORDERS_nextbar_low[FUNCGET_recordnumber]+";"+
         ORDERS_nextbar_close[FUNCGET_recordnumber]+";"+
         ORDERS_nextbar_highavg[FUNCGET_recordnumber]+";"+
         ORDERS_nextbar_lowavg[FUNCGET_recordnumber]+";"+
         ORDERS_nextbar_closeavg[FUNCGET_recordnumber]+";"+
         ORDERS_nextbar_closeavgdigital[FUNCGET_recordnumber]+";"+
      
         ORDERS_ticket[FUNCGET_recordnumber]+";"+
         ORDERS_openprice[FUNCGET_recordnumber]+";"+
         ORDERS_closeprice[FUNCGET_recordnumber]+";"+
...
void order_cleararray(){
   ...
   for(int FUNCVAR_counter=0;FUNCVAR_counter<100;FUNCVAR_counter++){
         ...
         ORDERS_thisbar_high[FUNCVAR_counter] = 0;
         ORDERS_thisbar_low[FUNCVAR_counter] = 0;
         ORDERS_thisbar_close[FUNCVAR_counter] = 0;
         ORDERS_nextbar_high[FUNCVAR_counter] = 0;
         ORDERS_nextbar_low[FUNCVAR_counter] = 0;
         ORDERS_nextbar_close[FUNCVAR_counter] = 0;
         ORDERS_nextbar_highavg[FUNCVAR_counter] = 0;
         ORDERS_nextbar_lowavg[FUNCVAR_counter] = 0;
         ORDERS_nextbar_closeavg[FUNCVAR_counter] = 0;
         ORDERS_nextbar_closeavgdigital[FUNCVAR_counter] = 0;
      
         ORDERS_ticket[FUNCVAR_counter] = 0;
         ORDERS_openprice[FUNCVAR_counter] = 0;
         ORDERS_closeprice[FUNCVAR_counter] = 0;
...
void order_addsignaturetoorder(int FUNCGET_target){
   ...
   ORDERS_pair[ORDERS_currentnumberoforders] = SIGN_pair;
   ORDERS_cmd[ORDERS_currentnumberoforders] = SIGN_cmd;
   ORDERS_spread[ORDERS_currentnumberoforders] = getinfo(502, SIGN_pair, GLOBAL_timeframe, ORDERS_barnumber);
   ORDERS_signature[ORDERS_currentnumberoforders] = SIGN_signature;
   ORDERS_target[ORDERS_currentnumberoforders] = GLOBAL_spreadfactor;
   
   ORDERS_thisbar_high[ORDERS_currentnumberoforders] = getinfo(2, SIGN_pair, GLOBAL_timeframe, ORDERS_barnumber);
   ORDERS_thisbar_low[ORDERS_currentnumberoforders] = getinfo(3, SIGN_pair, GLOBAL_timeframe, ORDERS_barnumber);
   ORDERS_thisbar_close[ORDERS_currentnumberoforders] = getinfo(4, SIGN_pair, GLOBAL_timeframe, ORDERS_barnumber);
   ORDERS_nextbar_high[ORDERS_currentnumberoforders] = getinfo(2, SIGN_pair, GLOBAL_timeframe, ORDERS_barnumber - 1);
   ORDERS_nextbar_low[ORDERS_currentnumberoforders] = getinfo(3, SIGN_pair, GLOBAL_timeframe, ORDERS_barnumber - 1);
   ORDERS_nextbar_close[ORDERS_currentnumberoforders] = getinfo(4, SIGN_pair, GLOBAL_timeframe, ORDERS_barnumber - 1);
   
   if(ORDERS_barnumber - 1 > 0){
      ORDERS_tested_price[ORDERS_currentnumberoforders] = getinfo(1, SIGN_pair, GLOBAL_timeframe, ORDERS_barnumber - 1);
      ORDERS_tested_endprice[ORDERS_currentnumberoforders] = getinfo(4, SIGN_pair, GLOBAL_timeframe, ORDERS_barnumber - 1);
   }
   
   if(SIGN_MINIBAR_RECENT_cases[FUNCGET_target] > 0){
      ORDERS_recent_rating[ORDERS_currentnumberoforders] = (SIGN_MINIBAR_RECENT_result[FUNCGET_target]+0.0) / (SIGN_MINIBAR_RECENT_cases[FUNCGET_target]+0.0);
      ORDERS_recent_cases[ORDERS_currentnumberoforders] = SIGN_MINIBAR_RECENT_cases[FUNCGET_target];
      ORDERS_recent_result[ORDERS_currentnumberoforders] = MathAbs(SIGN_MINIBAR_RECENT_result[FUNCGET_target]);
      ORDERS_recent_datemin[ORDERS_currentnumberoforders] = SIGN_MINIBAR_RECENT_datemin;
      ORDERS_recent_datemax[ORDERS_currentnumberoforders] = SIGN_MINIBAR_RECENT_datemax;
   }
   
   if(SIGN_MINIBAR_TOTAL_cases[FUNCGET_target] > 0){
      ORDERS_total_rating[ORDERS_currentnumberoforders] = (SIGN_MINIBAR_TOTAL_result[FUNCGET_target]+0.0) / (SIGN_MINIBAR_TOTAL_cases[FUNCGET_target]+0.0);
      ORDERS_total_cases[ORDERS_currentnumberoforders] = SIGN_MINIBAR_TOTAL_cases[FUNCGET_target];
      ORDERS_total_result[ORDERS_currentnumberoforders] = MathAbs(SIGN_MINIBAR_TOTAL_result[FUNCGET_target]);
   }
   
   if(SIGN_MINIBAR_NONINVERSE_cases[FUNCGET_target] > 0){
      ORDERS_noninverse_rating[ORDERS_currentnumberoforders] = (SIGN_MINIBAR_NONINVERSE_result[FUNCGET_target]+0.0) / (SIGN_MINIBAR_NONINVERSE_cases[FUNCGET_target]+0.0);
      ORDERS_noninverse_cases[ORDERS_currentnumberoforders] = SIGN_MINIBAR_NONINVERSE_cases[FUNCGET_target];
      ORDERS_noninverse_result[ORDERS_currentnumberoforders] = MathAbs(SIGN_MINIBAR_NONINVERSE_result[FUNCGET_target]);
   }

   if(SIGN_MINIBAR_PAIR_cases[FUNCGET_target] > 0){
      ORDERS_pair_rating[ORDERS_currentnumberoforders] = (SIGN_MINIBAR_PAIR_result[FUNCGET_target]+0.0) / (SIGN_MINIBAR_PAIR_cases[FUNCGET_target]+0.0);
      ORDERS_pair_cases[ORDERS_currentnumberoforders] = SIGN_MINIBAR_PAIR_cases[FUNCGET_target];
      ORDERS_pair_result[ORDERS_currentnumberoforders] = MathAbs(SIGN_MINIBAR_PAIR_result[FUNCGET_target]);
   }
   
   if(SIGN_NEXTBAR_countedbars > 0){
      ORDERS_nextbar_highavg[ORDERS_currentnumberoforders] = (SIGN_NEXTBAR_high+0.0) / (SIGN_NEXTBAR_countedbars+0.0);
      ORDERS_nextbar_lowavg [ORDERS_currentnumberoforders] = (SIGN_NEXTBAR_low+0.0) / (SIGN_NEXTBAR_countedbars+0.0);
      ORDERS_nextbar_closeavg[ORDERS_currentnumberoforders] = (SIGN_NEXTBAR_close+0.0) / (SIGN_NEXTBAR_countedbars+0.0);
      ORDERS_nextbar_closeavgdigital[ORDERS_currentnumberoforders] = (SIGN_NEXTBAR_closedigital+0.0) / (SIGN_NEXTBAR_countedbars+0.0);
   }
...
void order_addresultstoorder(){
   ...
   for(FUNCVAR_counter=0; FUNCVAR_counter < ORDERS_currentnumberoforders; FUNCVAR_counter++){
      if(ORDERS_cmd[FUNCVAR_counter] >=0){
         ...
         ORDERS_tested_targetdistance[FUNCVAR_counter] = ORDERS_spread[FUNCVAR_counter] * ORDERS_target[FUNCVAR_counter];
         ...
         if(ORDERS_cmd[FUNCVAR_counter] == 0){
            ...
            if(ORDERS_tested_targetdistance[FUNCVAR_counter] > 0){
...
void order_createorderarray(int FUNCGET_barnumber = 0){
   ...
   for(int a=0;a<CURRENCY_selected_numberofpairs;a++){
      ...
      signature_cleararray();
      signature_createsignaturearray(FUNCVAR_signature, FUNCVAR_currentpair, FUNCGET_barnumber);
...
void order_open(){
   ...
   for(FUNCVAR_count=0;FUNCVAR_count<ORDERS_currentnumberoforders;FUNCVAR_count++){
      if(ORDERS_cmd[FUNCVAR_count] >= 0){
         ...
         FUNCVAR_target = ORDERS_target[FUNCVAR_count] * ORDERS_spread[FUNCVAR_count];
         ...
         while(FUNCVAR_ticket < 0 && FUNCVAR_attempt < 6 && ORDERS_volume[FUNCVAR_count] > 0){
            if(ORDERS_cmd[FUNCVAR_count] == 1){
               ORDERS_price[FUNCVAR_count] = MarketInfo(ORDERS_pair[FUNCVAR_count], MODE_BID);
               ORDERS_stoploss[FUNCVAR_count] = ORDERS_price[FUNCVAR_count] + FUNCVAR_target;
               ORDERS_takeprofit[FUNCVAR_count] = ORDERS_price[FUNCVAR_count] - FUNCVAR_target;
            }else{
               ORDERS_price[FUNCVAR_count] = MarketInfo(ORDERS_pair[FUNCVAR_count], MODE_ASK);
               ORDERS_stoploss[FUNCVAR_count] = ORDERS_price[FUNCVAR_count] - FUNCVAR_target;
               ORDERS_takeprofit[FUNCVAR_count] = ORDERS_price[FUNCVAR_count] + FUNCVAR_target;
            }
      
            FUNCVAR_ticket = OrderSend(ORDERS_pair[FUNCVAR_count], ORDERS_cmd[FUNCVAR_count], ORDERS_volume[FUNCVAR_count], ORDERS_price[FUNCVAR_count], FUNCVAR_slippage, ORDERS_stoploss[FUNCVAR_count], ORDERS_takeprofit[FUNCVAR_count], ORDERS_comment[FUNCVAR_count]);
            if(FUNCVAR_ticket < 0){
...
int init(){  
   ...
   GLOBAL_timeframe              = HOUR;           //In Minutes. The timeframe to look at for signatures.
   ...
   if(GLOBAL_incminibar == false){
      ArrayResize(GLOBAL_targetpercentages, 1);
      GLOBAL_targetpercentages[0] = 0.25;
   }
   
   GLOBAL_isdemo                 = IsDemo();
   GLOBAL_riskpertradeamount     = AccountBalance() * GLOBAL_riskpertradepercentage;
   GLOBAL_exportorderarray_file  = AccountNumber()+"-ordersnew.csv";
...
void cron_update(){
   ...
   //Week
   if(
      iTime(Symbol(), PERIOD_W1, 0) - iTime(Symbol(), PERIOD_W1, 1) - TimeCurrent() < TIME_weekcrontime &&
      TIME_weekended == FALSE
   ){
      cron_endweek();
   }
   ...
   //Day
   if(
      iTime(Symbol(), PERIOD_D1, 0) - iTime(Symbol(), PERIOD_D1, 1) - TimeCurrent() < TIME_daycrontime &&
      TIME_dayended == FALSE
   ){
      cron_endday();
   }
   ...
   //Hour
   if(
      iTime(Symbol(), PERIOD_H1, 0) - iTime(Symbol(), PERIOD_H1, 1) - TimeCurrent() < TIME_hourcrontime &&
      TIME_hourended == FALSE
   ){
      cron_endhour();
   }
   ...
   //Increment
   if(
      iTime(Symbol(), GLOBAL_cronincrement, 0) - iTime(Symbol(), GLOBAL_cronincrement, 1) - TimeCurrent() < TIME_incrementcrontime &&
      TIME_incrementended == FALSE
   ){
      cron_endincrement();
   }
...
int init_getlookback(){
   ...
   for(FUNCVAR_counter=0;FUNCVAR_counter<CURRENCY_all_numberofpairs;FUNCVAR_counter++){
      ...
      while(
         ...
         iTime(FUNCVAR_currentpair, GLOBAL_minitimeframe, FUNCVAR_barnumber) > TimeCurrent() - GLOBAL_maxlookback
      ){
         FUNCVAR_barnumber++;
      }
...
void testing_demonumberofperiods(int FUNCGET_periods = 1, int FUNCGET_startbar = 1){
   ...
   for(FUNCVAR_counter = 1; FUNCVAR_counter <= FUNCGET_periods; FUNCVAR_counter++){
...
void testing_showorders(int FUNCGET_periods = 1, int FUNCGET_startbar = 2){
   function_start("testing_showorders", true);
   ...
   log("Running testing_showorders");
   ...
   for(FUNCVAR_counter = 0; FUNCVAR_counter <= FUNCGET_periods; FUNCVAR_counter++){
      ...
      for(int a=0;a<CURRENCY_selected_numberofpairs;a++){
         FUNCVAR_currentpair = CURRENCY_selected_pairs[a];
         FUNCVAR_signature = signature_getbarsignature(FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_bar);
         ORDERS_barnumber = FUNCVAR_bar;
         ORDERS_date = iTime(FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_bar);
         signature_cleararray();
         signature_createsignaturearray(FUNCVAR_signature, FUNCVAR_currentpair, FUNCVAR_bar);
         signature_dumpsignaturearray();
         order_addsignaturetoorder(0);
      }
      order_exportorderarray();
   }
   
   log("Finished testing_showorders");
   function_end();
}
...
void testing_outputeachsignature(string FUNCGET_command = "NULL"){
   ...
   log("testing_outputoverview;"+
      "FUNCVAR_signature;"+
      "SIGN_MINIBAR_TOTAL_cases[5];"+
      "SIGN_MINIBAR_TOTAL_result[5];"+
      "SIGN_MINIBAR_NONINVERSE_cases[5];"+
      "SIGN_MINIBAR_NONINVERSE_result[5];"+
      "SIGN_MINIBAR_PAIR_cases[5];"+
      "SIGN_MINIBAR_PAIR_result[5];"+
      "SIGN_MINIBAR_RECENT_cases[5];"+
      "SIGN_MINIBAR_RECENT_result[5];");     
   for(int a=1;a<6;a++){
      for(int b=1;b<6;b++){
         for(int c=1;c<6;c++){
            for(int d=1;d<6;d++){
               ...
               signature_createsignaturearray(FUNCVAR_signature, "GBPUSD");
               log("testing_outputoverview;"+
               FUNCVAR_signature+";"+
               SIGN_MINIBAR_TOTAL_cases[5]+";"+
               SIGN_MINIBAR_TOTAL_result[5]+";"+
               ...
               SIGN_MINIBAR_NONINVERSE_result[5]+";"+
               SIGN_MINIBAR_RECENT_cases[5]+";"+
               SIGN_MINIBAR_RECENT_result[5]); //Log the 5th target in the for this signature.
            }
         }
      }
   }
