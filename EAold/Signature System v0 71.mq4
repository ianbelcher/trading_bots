...
extern int     GLOBAL_goalminnumberofcases   = 10;             //There needs to be at least this many cases of a previous signature for it to be counted.
...
extern string  CURRENCY_skippairs            = "";
...
int
   ...
   CURRENCY_pairslookback[100]
   ;
string
   CURRENCY_entities[100],
   CURRENCY_pairs[100]
   ;
//^
...
int init(){  
   ...
   GLOBAL_recentlookback         = 60 * MONTH * 6; //In Seconds. For the RECENT sub-signature, this is the lookback.
...
void signature_setsignaturearray(string FUNCGET_signature, string FUNCGET_pair, int FUNCGET_barnumber = 0){
   ...
   for(a=0;a<CURRENCY_numberofpairs;a++){
      FUNCVAR_currentpair = CURRENCY_pairs[a];
       for(FUNCVAR_barnumber=FUNCGET_barnumber + 2;FUNCVAR_barnumber < CURRENCY_pairslookback[a]-2;FUNCVAR_barnumber++){ //Start looking at data 2 bars back from the bar we're looking at and 2 bars before the end of the data
         ...
         if(
            ...
            for(FUNCVAR_targetcounter=0;FUNCVAR_targetcounter < ArraySize(GLOBAL_targetpercentages); FUNCVAR_targetcounter++){
               ...
               if(getinfo(5, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumber) > getinfo(5, FUNCVAR_currentpair, GLOBAL_timeframe, FUNCGET_barnumber) - GLOBAL_recentlookback){
...
void signature_addsignaturetosum(int FUNCGET_target, string FUNCGET_pair){
   ...
   SIGNATURE_SUM_signature[GLOBAL_currentnumberoforders] = SIGNATURE_TOTAL_signature[FUNCGET_target];
   ...
   SIGNATURE_SUM_rating[GLOBAL_currentnumberoforders] = (SIGNATURE_TOTAL_result[FUNCGET_target]+0.0) / (SIGNATURE_TOTAL_cases[FUNCGET_target]+0.0);
   SIGNATURE_SUM_cases[GLOBAL_currentnumberoforders] = SIGNATURE_TOTAL_cases[FUNCGET_target];
   SIGNATURE_SUM_result[GLOBAL_currentnumberoforders] = MathAbs(SIGNATURE_TOTAL_result[FUNCGET_target]);
   SIGNATURE_SUM_noninverse_rating[GLOBAL_currentnumberoforders] = ((SIGNATURE_NONINVERSE_result[FUNCGET_target]+0.0) / (SIGNATURE_NONINVERSE_cases[FUNCGET_target]+0.0000000000001));
   SIGNATURE_SUM_pair_rating[GLOBAL_currentnumberoforders] = ((SIGNATURE_PAIR_result[FUNCGET_target]+0.0) / (SIGNATURE_PAIR_cases[FUNCGET_target]+0.0000000000001));
   SIGNATURE_SUM_recent_rating[GLOBAL_currentnumberoforders] = ((SIGNATURE_RECENT_result[FUNCGET_target]+0.0) / (SIGNATURE_RECENT_cases[FUNCGET_target]+0.0000000000001));
   
   SIGNATURE_SUM_recent_result[GLOBAL_currentnumberoforders] = SIGNATURE_RECENT_result[FUNCGET_target];
...
void signature_dumpsumarray(){
   ...
   log(
      ...
      "SIGNATURE_SUM_pair"+";"+
      "getinfo(502)"+";"+
      "SIGNATURE_SUM_signature"+";"+
      "SIGNATURE_SUM_target"+";"+
      "SIGNATURE_SUM_targetdistance"+";"+
      "SIGNATURE_SUM_targetpositive"+";"+
      "SIGNATURE_SUM_targetnegative"+";"+
      "SIGNATURE_SUM_order"+";"+
      "SIGNATURE_SUM_result"+";"+
      "SIGNATURE_SUM_rating"+";"+
      "SIGNATURE_SUM_cases"+";"+
      "SIGNATURE_SUM_noninverse_rating"+";"+
      "SIGNATURE_SUM_pair_rating"+";"+
      "SIGNATURE_SUM_recent_rating"+";"+
      "SIGNATURE_SUM_recent_result"+";"+
      "SIGNATURE_SUM_recent_cases"+";"+
      "SIGNATURE_SUM_win"
   );
   for(FUNCVAR_counter=0; FUNCVAR_counter < GLOBAL_currentnumberoforders; FUNCVAR_counter++){
...
void init_populatecurrencyarray(){
   ...
   if(CURRENCY_USD){
      CURRENCY_entities[0] = "USD";
   }   
   if(CURRENCY_CHF){
      CURRENCY_entities[1] = "CHF";
   }   
   if(CURRENCY_EUR){
      CURRENCY_entities[2] = "EUR";
   }   
   if(CURRENCY_GBP){
      CURRENCY_entities[3] = "GBP";
   }
   if(CURRENCY_CAD){
      CURRENCY_entities[4] = "CAD";
   }   
   if(CURRENCY_JPY){
      CURRENCY_entities[5] = "JPY";
   }   
   if(CURRENCY_AUD){
      CURRENCY_entities[6] = "AUD";
   }   
   if(CURRENCY_NZD){
      CURRENCY_entities[7] = "NZD";
   }
   if(CURRENCY_SGD){
      CURRENCY_entities[8] = "SGD";
   }   
   if(CURRENCY_HKD){
      CURRENCY_entities[9] = "HKD";
   }   
   if(CURRENCY_DKK){
      CURRENCY_entities[10] = "DKK";
   }   
   if(CURRENCY_NOK){
      CURRENCY_entities[11] = "NOK";
   }
   if(CURRENCY_SEK){
      CURRENCY_entities[12] = "SEK";
   }   
   if(CURRENCY_TRY){
      CURRENCY_entities[13] = "TRY";
   }   
   if(CURRENCY_PLN){
      CURRENCY_entities[14] = "PLN";
   }   
   if(CURRENCY_MXN){
      CURRENCY_entities[15] = "MXN";
   }
   if(CURRENCY_XAU){
      CURRENCY_entities[16] = "XAU";
   }   
   if(CURRENCY_XAG){
      CURRENCY_entities[17] = "XAG";
   }      
   ...
   for(FUNCVAR_basecurrency = 0; FUNCVAR_basecurrency < 100; FUNCVAR_basecurrency++){
      for(FUNCVAR_countercurrency = 0; FUNCVAR_countercurrency < 100; FUNCVAR_countercurrency++){
         FUNCVAR_currentpair = CURRENCY_entities[FUNCVAR_basecurrency] + CURRENCY_entities[FUNCVAR_countercurrency];
         if(
   ...
   log("Found "+CURRENCY_numberofpairs+" pairs: "+FUNCVAR_currencylist);
...
int init_getlookback(){
   ...
   for(int a=0;a<CURRENCY_numberofpairs;a++){
      ...
      CURRENCY_pairslookback[a] = iBarShift(FUNCVAR_currentpair, GLOBAL_timeframe, iTime(FUNCVAR_currentpair, GLOBAL_minitimeframe, FUNCVAR_barnumber - 1)) - 1; //Get equiv in the main timeframe then minus 1.
      log(FUNCVAR_currentpair+" back to barnumber "+CURRENCY_pairslookback[a]+" dated: "+humandate(iTime(FUNCVAR_currentpair, GLOBAL_timeframe, CURRENCY_pairslookback[a])));
   }
   GetLastError();
...
void testing_runtests(){
   ...
   //Returns a report on the trades which would have been taken in the last number of days
      testing_outputdemonumberofdays(100);
...
void testing_outputsignaturedata(){
   ...
   for(int a=0;a<CURRENCY_numberofpairs;a++){
      FUNCVAR_currentpair = CURRENCY_pairs[a];
      for(FUNCVAR_barnumber=2;FUNCVAR_barnumber < CURRENCY_pairslookback[a];FUNCVAR_barnumber++){        
         //v Construct the basic signature for this reference
         FUNCVAR_signature = signature_getbarsignature(FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumber + 1) + signature_getbarsignature(FUNCVAR_currentpair, GLOBAL_timeframe, FUNCVAR_barnumber);
         //^
